...
public class OrnamentsExample {
		...
		for (int divisor = 0; divisor < interesting.size(); divisor++) {
			...
			λImage result = (x, y) -> {
				...
				if (pattern.valueAt(X, Y)) {
					return Colors.ORANGE().customize().mutliply(0.9f);
				} else {
					return Colors.BLACK();
				}
			};
	...
	static λBinaryImageOperation OR = (a, b) -> (x, y) -> a.valueAt(x, y) || b.valueAt(x, y);
	static λBinaryImageOperation AND = (a, b) -> (x, y) -> a.valueAt(x, y) && b.valueAt(x, y);

	private static boolean isPrime(int x) {
		return prime_numbers.contains(x);
	}

	private static λImage option1(Rectangle image_dimentions) {
		int W = (int) image_dimentions.getWidth();
		int H = (int) image_dimentions.getHeight();
		...
		return (a, b) -> {
			...
			// --- Раскрашиваем--------------------------
			// if (() && (prime_numbers.contains(Y))) {
			// return Colors.PURPLE();
			// }

			if ((Xp == 0 || Yp == 0) && (i == 0 || j == 0)) {
				if (prime_numbers.contains(x) || prime_numbers.contains(y)) {
					return Colors.PURPLE();
				}
			}
